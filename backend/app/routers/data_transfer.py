...
_FONTS_DIR = (_HERE / ".." / ".." / "assets" / "fonts" / "custom").resolve()
...
def _export_fonts(db: Session) -> List[Dict[str, Any]]:
    """
    Exporta cada fuente personalizada con sus archivos .ttf/.otf codificados
    en base64 (no son texto plano como el resto de los módulos, pero viajan
    igual dentro del mismo JSON). Así, al sincronizar, la otra persona recibe
    los archivos reales, no solo el nombre — no necesita subirlos de nuevo.
    """
    out = []
    for f in db.query(CustomFont).all():
        row = {"name": f.name, "css_key": f.css_key}
        for campo, path_str in (("regular", f.regular_path), ("bold", f.bold_path)):
            if path_str and Path(path_str).exists():
                p = Path(path_str)
                row[f"{campo}_ext"] = p.suffix.lstrip(".")
                row[f"{campo}_b64"] = base64.b64encode(p.read_bytes()).decode("ascii")
        out.append(row)
    return out
...
def _import_fonts(db: Session, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    rep = _report()
    for row in rows:
        try:
            css_key = row.get("css_key")
            name = row.get("name")
            if not css_key or not name:
                rep["errores"].append("fuente omitida: falta name/css_key")
                continue

            existing = db.query(CustomFont).filter(CustomFont.css_key == css_key).first()
            font = existing or CustomFont(name=name, css_key=css_key)

            for campo in ("regular", "bold"):
                b64 = row.get(f"{campo}_b64")
                ext = row.get(f"{campo}_ext", "ttf")
                if not b64:
                    continue
                dest = _FONTS_DIR / f"{css_key}_{campo}.{ext}"
                dest.write_bytes(base64.b64decode(b64))
                setattr(font, f"{campo}_path", str(dest))
...
IMPORT_ORDER = ["companies", "services", "users", "clients", "opportunities", "quotations", "fonts"]
